...
import { getAPPOverview } from '@/services/statistics'
...
const AllEventsRing = ({ FullEvents }) => {
  return <RingChart data={FullEvents} sumText="全部事件" />
}
...
const OverView = ({ appId }) => {
  const [eventAPI, setEventAPI] = useState([])
  const [fullEvents, setFullEvents] = useState([])
  const fetchAPPOverview = async () => {
    const {
      Code,
      Data: { EventAPI, FullEvents },
    } = await getAPPOverview({ AppId: appId })
  ...
  return (
    <div className={styles.overViewWrapper}>
      <ProCard gutter={6} className={styles.overViewMain} bodyStyle={{ padding: 0 }}>
        <ProCard colSpan={12} layout="center" bodyStyle={{ height: 140, padding: 0 }}>
          <AffectedCountPane EventAPI={eventAPI} />
        </ProCard>
        <ProCard colSpan={12} layout="center" bodyStyle={{ height: 140, padding: 0 }}>
          <AllEventsRing FullEvents={fullEvents} />
        </ProCard>
      </ProCard>
    </div>
  )
