...
import { getApiEvents, modifyApiRiskStatus } from '@/services/apiPortrait'
...
export default function TableRisks() {
  ...
  const columns = [
    {
      ...
      render: (RiskName, { RiskId }) => {
        return (
          <div style={{ cursor: 'pointer', color: '#1890ff' }} onClick={() => showDetails(RiskId)}>
            {RiskName}
          </div>
        )
      },
  ...
  const onModifyStatus = (targetStatus) => {
    ...
    Modal.confirm({
      ...
      closable: true,
      onOk: async () => {
        const RiskIds = selectedRecords.map((e) => e.RiskId)
        const { Code } = await modifyApiRiskStatus({
          RiskIds,
          Status: targetStatus,
        })
        if (Code !== 'Succeed') return
